...
async def calculate_indicator(settings: IndicatorSettings):
    ...
    if settings.indicator_type == "dominant" and DOMINANT_AVAILABLE:
        ...
        try:
            ...
            long_trades_list = [t for t in trades if t.get('direction') == 1 or str(t.get('direction', '')).upper() == 'LONG']
            short_trades_list = [t for t in trades if t.get('direction') == -1 or str(t.get('direction', '')).upper() == 'SHORT']
            
            long_trades_count = len(long_trades_list)
            short_trades_count = len(short_trades_list)
            long_wins = len([t for t in long_trades_list if t.get('pnl_percent', 0) > 0])
            short_wins = len([t for t in short_trades_list if t.get('pnl_percent', 0) > 0])
            ...
            for trade in trades:
                ...
                # Convert boolean tps_hit to list of TP numbers [1, 2, 3, 4]
                tps_hit_bool = trade.get('tps_hit', [])
                tps_hit_nums = [i + 1 for i, hit in enumerate(tps_hit_bool) if hit]
            ...
            # Calculate TP accuracy breakdown for Dominant
            accuracy = {}
            for n in [5, 10, 20, 50, 100]:
                recent = adapted_trades[-n:] if len(adapted_trades) >= n else adapted_trades
                accuracy[f"last_{n}"] = {}
                for tp_num in range(1, 5):  # TP1-TP4 for Dominant
                    hits = sum(1 for t in recent if tp_num in t.get('tps_hit', []))
                    accuracy[f"last_{n}"][f"tp{tp_num}"] = round(hits / len(recent) * 100, 1) if recent else 0
            
            # Overall accuracy
            accuracy["total"] = {}
            for tp_num in range(1, 5):
                hits = sum(1 for t in adapted_trades if tp_num in t.get('tps_hit', []))
                accuracy["total"][f"tp{tp_num}"] = round(hits / len(adapted_trades) * 100, 2) if adapted_trades else 0
...
async def auto_optimize_stream_get(
    ...
    if indicator_type == "dominant":
        async def err():
            yield 'data: {"type": "error", "message": "Optimization for Dominant not yet implemented. Use presets library."}\n\n'
        return StreamingResponse(err(), media_type="text/event-stream")
    
    settings = IndicatorSettings(
        symbol=symbol, timeframe=timeframe, indicator_type=indicator_type,
        trg_atr_length=i1, trg_multiplier=i2,
        dominant_sensitivity=dominant_sensitivity,
        dominant_filter_type=dominant_filter_type,
        dominant_sl_mode=dominant_sl_mode,
        tp_count=tp_count,
        tp1_percent=tp1, tp2_percent=tp2, tp3_percent=tp3, tp4_percent=tp4,
        sl_percent=sl, sl_trailing_mode=sl_mode,
        use_supertrend=use_st, supertrend_period=st_period, supertrend_multiplier=st_mult,
        use_rsi_filter=use_rsi, rsi_period=rsi_period,
        use_adx_filter=use_adx, adx_threshold=adx_threshold,
        allow_reentry=allow_reentry, leverage=leverage,
        use_commission=use_commission, commission_percent=commission,
        start_date=start_date, end_date=end_date,
    )
    request = AutoOptimizeMode(mode=mode, settings=settings, metric=metric, full_mode_depth=full_mode_depth)
