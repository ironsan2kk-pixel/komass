...
@router.get("/presets/stream")
async def stream_optimization_get(
    pairs: str = Query(..., description="Comma-separated list of pairs (e.g., BTCUSDT,ETHUSDT)"),
    timeframe: str = Query(default="1h", description="Timeframe"),
    mode: str = Query(default="quick", description="Mode: quick/standard/full"),
    indicator_type: str = Query(default="", description="Filter by indicator: trg/dominant/empty for all"),
    start_date: Optional[str] = Query(None, description="Start date YYYY-MM-DD"),
    end_date: Optional[str] = Query(None, description="End date YYYY-MM-DD"),
):
    """
    GET version of preset optimization stream for EventSource compatibility.
    ...
    if not pairs_list:
        async def error_gen():
            yield f'data: {json.dumps({"type": "error", "message": "No pairs provided"})}\n\n'
    ...
    try:
        ...
        query = "SELECT id, name, indicator_type FROM presets WHERE is_active = 1"
        params = []
