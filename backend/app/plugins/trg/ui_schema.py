...
class TRGUISchema:
    ...
    def _build_take_profit_section(self) -> UISection:
        ...
        tp_colors = [
            ...
            "#ec4899", "#eab308", "#84cc16", "#6366f1", "#14b8a6",
        ]
    ...
    def _build_filters_section(self) -> UISection:
        ...
        return UISection(
            ...
            fields=[
                UIField(id="use_supertrend", type=FieldType.CHECKBOX, label="SuperTrend", default=False),
                UIField(id="supertrend_period", type=FieldType.NUMBER, label="ST Period", default=10, min=5, max=50, step=1, depends_on="use_supertrend", depends_value=True),
                UIField(id="supertrend_multiplier", type=FieldType.NUMBER, label="ST Multiplier", default=3.0, min=1.0, max=10.0, step=0.5, depends_on="use_supertrend", depends_value=True),
                UIField(id="use_rsi_filter", type=FieldType.CHECKBOX, label="RSI Filter", default=False),
                UIField(id="rsi_period", type=FieldType.NUMBER, label="RSI Period", default=14, min=5, max=50, step=1, depends_on="use_rsi_filter", depends_value=True),
                UIField(id="rsi_overbought", type=FieldType.NUMBER, label="Overbought", default=70, min=50, max=100, step=1, depends_on="use_rsi_filter", depends_value=True),
                UIField(id="rsi_oversold", type=FieldType.NUMBER, label="Oversold", default=30, min=0, max=50, step=1, depends_on="use_rsi_filter", depends_value=True),
                UIField(id="use_adx_filter", type=FieldType.CHECKBOX, label="ADX Filter", default=False),
                UIField(id="adx_period", type=FieldType.NUMBER, label="ADX Period", default=14, min=5, max=50, step=1, depends_on="use_adx_filter", depends_value=True),
                UIField(id="adx_threshold", type=FieldType.NUMBER, label="ADX Threshold", default=25, min=10, max=50, step=1, depends_on="use_adx_filter", depends_value=True),
                UIField(id="use_volume_filter", type=FieldType.CHECKBOX, label="Volume Filter", default=False),
                UIField(id="volume_ma_period", type=FieldType.NUMBER, label="Volume MA Period", default=20, min=5, max=100, step=1, depends_on="use_volume_filter", depends_value=True),
                UIField(id="volume_threshold", type=FieldType.NUMBER, label="Volume Threshold", default=1.5, min=1.0, max=5.0, step=0.1, suffix="x", depends_on="use_volume_filter", depends_value=True),
            ]
        )
    ...
    def _build_reentry_section(self) -> UISection:
        ...
        return UISection(
            ...
            default_open=False,
            fields=[
                UIField(id="allow_reentry", type=FieldType.CHECKBOX, label="Разрешить повторный вход", default=False),
                UIField(id="reentry_after_sl", type=FieldType.CHECKBOX, label="После SL", default=True, depends_on="allow_reentry", depends_value=True),
                UIField(id="reentry_after_tp", type=FieldType.CHECKBOX, label="После TP", default=True, depends_on="allow_reentry", depends_value=True),
                UIField(id="reentry_delay_bars", type=FieldType.NUMBER, label="Задержка (свечей)", default=1, min=0, max=10, step=1, depends_on="allow_reentry", depends_value=True),
            ]
        )
    ...
    def _build_adaptive_section(self) -> UISection:
        ...
        return UISection(
            ...
            fields=[
                ...
                UIField(id="adaptive_lookback", type=FieldType.NUMBER, label="Lookback (сделок)", default=20, min=10, max=100, step=5, depends_on="adaptive_mode"),
                UIField(id="adaptive_recalc_trades", type=FieldType.NUMBER, label="Пересчёт каждые", default=20, min=5, max=50, step=5, suffix="сделок", depends_on="adaptive_mode"),
            ]
        )
    ...
    def _build_capital_section(self) -> UISection:
        ...
        return UISection(
            ...
            default_open=False,
            fields=[
                UIField(id="initial_capital", type=FieldType.NUMBER, label="Начальный капитал", default=10000, min=100, max=10000000, step=100, suffix="$", format="currency"),
                UIField(
                ...
                UIField(id="position_size_value", type=FieldType.NUMBER, label="Значение", default=100, min=1, max=100, step=1, suffix="%"),
            ]
        )
    ...
    def _build_data_section(self) -> UISection:
        ...
        return UISection(
            ...
            fields=[
                UIField(id="symbol", type=FieldType.SELECT, label="Пара", default="BTCUSDT", options=[]),
                UIField(
                    id="timeframe",
                    type=FieldType.SELECT,
                    label="Таймфрейм",
    ...
    def _build_tabs(self) -> List[UITab]:
        """Build all tabs"""
        return [
            UITab(id="chart", label="График", icon="📈", type=TabType.CHART, order=1, default=True, component="ChartPanel"),
            UITab(id="stats", label="Статистика", icon="📊", type=TabType.STATS, order=2, component="StatsPanel"),
            UITab(id="trades", label="Сделки", icon="📋", type=TabType.TABLE, order=3, component="TradesTable"),
            UITab(id="monthly", label="Месяцы", icon="📅", type=TabType.PANEL, order=4, component="MonthlyPanel"),
            UITab(id="optimizer", label="Оптимизация", icon="🔥", type=TabType.PANEL, order=5, component="AutoOptimizePanel"),
            UITab(id="heatmap", label="Heatmap", icon="🗺️", type=TabType.PANEL, order=6, component="HeatmapPanel"),
        ]
    ...
    def get_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sidebar": {"sections": [s.to_dict() for s in self._sections]},
    ...
    def get_sidebar_schema(self) -> Dict[str, Any]:
        return {"sections": [s.to_dict() for s in self._sections]}
    ...
    def get_optimization_ranges(self) -> Dict[str, Dict[str, Any]]:
        ...
        ranges = {}
        ranges["trg_atr_length"] = {"min": 10, "max": 200, "step": 5, "default": 45, "optimization_range": [20, 30, 35, 40, 45, 50, 55, 60, 70, 80, 100, 120]}
        ranges["trg_multiplier"] = {"min": 1.0, "max": 10.0, "step": 0.5, "default": 4.0, "optimization_range": [1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6, 7, 8]}
        for i in range(1, 11):
            ranges[f"tp{i}_percent"] = {"min": 0.1, "max": 100.0, "step": 0.5, "default": 1.0}
            ranges[f"tp{i}_amount"] = {"min": 0, "max": 100, "step": 5, "default": 25}
        ranges["sl_percent"] = {"min": 1.0, "max": 50.0, "step": 0.5, "default": 8.0, "optimization_range": [3, 4, 5, 6, 7, 8, 10, 12, 15]}
        return ranges
    
    def get_filter_presets(self) -> List[Dict[str, Any]]:
        """Get predefined filter presets."""
        return [
            {"name": "No Filters", "use_supertrend": False, "use_rsi_filter": False, "use_adx_filter": False, "use_volume_filter": False},
            {"name": "SuperTrend Only", "use_supertrend": True, "supertrend_period": 10, "supertrend_multiplier": 3.0, "use_rsi_filter": False, "use_adx_filter": False, "use_volume_filter": False},
            {"name": "RSI Filter", "use_supertrend": False, "use_rsi_filter": True, "rsi_period": 14, "rsi_overbought": 70, "rsi_oversold": 30, "use_adx_filter": False, "use_volume_filter": False},
            {"name": "Trend + Momentum", "use_supertrend": True, "supertrend_period": 10, "supertrend_multiplier": 3.0, "use_rsi_filter": True, "rsi_period": 14, "rsi_overbought": 70, "rsi_oversold": 30, "use_adx_filter": True, "adx_period": 14, "adx_threshold": 25, "use_volume_filter": False},
            {"name": "Full Filters", "use_supertrend": True, "supertrend_period": 10, "supertrend_multiplier": 3.0, "use_rsi_filter": True, "rsi_period": 14, "rsi_overbought": 70, "rsi_oversold": 30, "use_adx_filter": True, "adx_period": 14, "adx_threshold": 25, "use_volume_filter": True, "volume_ma_period": 20, "volume_threshold": 1.5},
        ]
    
    def get_tp_presets(self) -> List[Dict[str, Any]]:
        """Get predefined TP configuration presets."""
        return [
            {"name": "Conservative", "tp_count": 4, "tp1_percent": 0.8, "tp1_amount": 50, "tp2_percent": 1.5, "tp2_amount": 30, "tp3_percent": 2.5, "tp3_amount": 15, "tp4_percent": 4.0, "tp4_amount": 5},
            {"name": "Balanced (Default)", "tp_count": 4, "tp1_percent": 1.05, "tp1_amount": 50, "tp2_percent": 1.95, "tp2_amount": 30, "tp3_percent": 3.75, "tp3_amount": 15, "tp4_percent": 6.0, "tp4_amount": 5},
            {"name": "Aggressive", "tp_count": 4, "tp1_percent": 1.5, "tp1_amount": 40, "tp2_percent": 3.0, "tp2_amount": 30, "tp3_percent": 5.0, "tp3_amount": 20, "tp4_percent": 8.0, "tp4_amount": 10},
            {"name": "Scalping", "tp_count": 3, "tp1_percent": 0.3, "tp1_amount": 60, "tp2_percent": 0.6, "tp2_amount": 30, "tp3_percent": 1.0, "tp3_amount": 10},
            {"name": "Swing", "tp_count": 4, "tp1_percent": 2.0, "tp1_amount": 40, "tp2_percent": 5.0, "tp2_amount": 30, "tp3_percent": 10.0, "tp3_amount": 20, "tp4_percent": 20.0, "tp4_amount": 10},
        ]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "json":
            ...
            test_settings = {"trg_atr_length": 45, "trg_multiplier": 4.0, "tp_count": 4, "sl_percent": 8.0}
